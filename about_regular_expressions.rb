...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    #
    # Since * will always accept the empty string, it will only
    #   fail if you have something before or after it that doesn't 
    #   match. For example, ab*c will fail to match azc, since b* will not match z and c will not match zc.
  end
  ...
  # Why?
  #   From RubyDocs:
  # Repetition is greedy by default: as many occurrences as possible are matched while still 
  #   allowing the overall match to succeed. By contrast, lazy matching makes the minimal amount
  #   of matches necessary for overall success. A greedy metacharacter can be made lazy by following it with ?.
  # Extra:
  #   A quantifier followed by + matches possessively: once it has matched it does not backtrack. 
  #    They behave like greedy quantifiers, but having matched they refuse to “give up” their match 
